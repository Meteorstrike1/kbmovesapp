...
class TestModuleTwo(TestAPI):
    """Testing Module 2 table"""
    ...
    def test_get_moves_in_combination_by_id_success(self):
        ...
        status_code = get_response.status_code
        expected = "turning backfist"
        result = get_response.json[0]["name"]

        self.assertEqual(status_code, 200)
        self.assertEqual(expected, result)

    def test_get_combination_by_id_none_fail(self):
        """List of moves in combination by id, not found"""
        id = 99
        get_response = self.client.get(f"{endpoint}/combo_moves/{id}")
        status_code = get_response.status_code
        expected = {'message': f'No combination moves. You have requested this URI [/module_2/combo_moves/{id}] '
                               f'but did you mean /module_2/combo_moves/<string:code> '
                               f'or /module_1/combo_moves/<string:code> or /module_2/code/<string:code> ?'}
    ...
    def test_get_combination_by_missing_id_fail(self):
        """List of moves in combination by id that doesn't exist in data"""
        id = 26
        get_response = self.client.get(f"{endpoint}/combo_moves/{id}")
        status_code = get_response.status_code
    ...
    def test_get_combination_by_id_missing_fail(self):
        """List of moves in combination by id, not found"""
        id = 3
        get_response = self.client.get(f"{endpoint}/combo_moves/{id}")
        status_code = get_response.status_code
        expected = {'message': f'Combination moves could not be found. You have requested this URI '
                               f'[/module_2/combo_moves/{id}] but did you mean /module_2/combo_moves/<string:code> '
                               f'or /module_1/combo_moves/<string:code> or /module_2/code/<string:code> ?'}
        result = get_response.json
    ...
    def test_get_combination_by_code_none_fail(self):
        ...
        get_response = self.client.get(f"{endpoint}/combo_moves/{code}")
        status_code = get_response.status_code
        expected = {'message': f'No combination moves. You have requested this URI [/module_2/combo_moves/{code}] '
                               f'but did you mean /module_2/combo_moves/<string:code> '
                               f'or /module_1/combo_moves/<string:code> or /module_2/code/<string:code> ?'}
        result = get_response.json
    ...
    def test_get_combination_by_code_missing_fail(self):
        ...
        status_code = get_response.status_code
        expected = {'message': f'Combination moves could not be found. You have requested this URI '
                               f'[/module_2/combo_moves/{code}] but did you mean /module_2/combo_moves/<string:code> '
                               f'or /module_1/combo_moves/<string:code> or /module_2/code/<string:code> ?'}
        result = get_response.json
